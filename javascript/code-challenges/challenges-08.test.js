...
const sortStarWarsCharacters = (starWarsArr) => {
  // Sort the starWarsArr based on height in descending order
...
const removeThree = (idx, arr) => {
  // Solution code here...
  arr.splice(idx, 3); // Remove three items starting from the index 'idx'
...
const joinArray = (arr) => {
  return arr.join(' '); // Join array elements with a space as the separator
};
/* ------------------------------------------------------------------------------------------------
CHALLENGE 4
...
const howMuchPencil = (str) => {
  ...
  for (let i = 0; i <= str.length; i++) {
    result.push(str.slice(i)); // Push the sliced string into the result array
    return result;
};
}
/* ------------------------------------------------------------------------------------------------
...
const listFoods = (recipe) => {
  ...
  recipe.ingredients.forEach(ingredient => {
    ...
    const foodName = ingredient.slice(spaceIndex + 1);

    // Remove any additional unit descriptors (e.g., pounds, gallons, cups, etc.)
    const unitDescriptors = ['pound', 'pounds', 'gallon', 'gallons', 'cup', 'cups'];
    unitDescriptors.forEach(unit => {
      if (foodName.includes(unit)) {
        const unitIndex = foodName.indexOf(unit);
        result.push(foodName.slice(unitIndex + unit.length + 1));
      }
    });
  });
...
const splitFoods = (recipe) => {
  ...
  recipe.ingredients.forEach(ingredient => {
    ...
    const foodNameIndex = words.slice(2).join(' ').indexOf(' ') + 3;
    const foodName = words.slice(foodNameIndex).join(' ');
...
const removeEvenValues = (arr) => {
  for (let i = 0; i < arr.length; ) {
    if (arr[i] % 2 === 0) {
      arr.splice(i, 1); // Remove even values by index
    } else {
      i++; // Move to the next element if it's not even
    }
  }
};
/* ------------------------------------------------------------------------------------------------
...
const removeLastCharacters = (str, numberOfCharacters) => {
  if (numberOfCharacters < 0) {
    return str; // Return the input string if numberOfCharacters is negative
  } else if (numberOfCharacters >= str.length) {
    return ''; // Return an empty string if numberOfCharacters is greater than or equal to the string length
  } else {
    return str.slice(0, -numberOfCharacters); // Remove numberOfCharacters from the end of the string
  }
};
/* ------------------------------------------------------------------------------------------------
...
const totalSumCSV = (str) => {
  ...
  // Loop through each value, convert it to a number, and add it to the total
  for (let i = 0; i < values.length; i++) {
    const num = Number(values[i]); // Convert value to a number
...
const removeVowels = (str) => {
  // Use a regular expression to replace all vowels (case-insensitive) with an empty string
...
const extractVowels = (str) => {
  const vowels = str.match(/[aeiou]/gi); // Find all vowels using regex
  const removedVowels = vowels ? vowels.sort().join('') : ''; // Sort and join vowels

  // Remove vowels from the original string
  const stringWithoutVowels = str.replace(/[aeiou]/gi, '');

  return [stringWithoutVowels, removedVowels];
};
/* ------------------------------------------------------------------------------------------------
TESTS
