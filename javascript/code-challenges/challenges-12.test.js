...
function transformToLis(obj) {
  ...
  const listItems = keyValuePairs.map(([key, value]) => `<li>${key}: ${value}</li>`);
  return listItems;
}
...
const count = (target, input) => {
  ...
  // Use reduce again to count occurrences of the target integer in the flattened array
  const totalCount = flattenedArray.reduce((count, num) => {
    if (num === target) {
      return count + 1;
    }
    return count;
...
const totalSum = (input) => {
  // Use reduce to iterate through the array of arrays and calculate the sum
  const sum = input.reduce((total, arr) => {
    // Use reduce on each inner array to calculate the sum of its elements
    const innerSum = arr.reduce((acc, num) => acc + num, 0);
    return total + innerSum; // Add the sum of the inner array to the total
  }, 0);
...
const divisibleByFiveTwoToThePower = (input) => {
  const filteredArray = input.map(subArray => {
    return subArray.filter(num => typeof num === 'number' && num % 5 === 0);
  });

  const poweredArray = filteredArray.map(subArray => {
    return subArray.map(num => Math.pow(2, num));
  });
...
let findMaleAndFemale = (data) => {
  const maleAndFemaleCharacters = data.reduce((acc, character) => {
