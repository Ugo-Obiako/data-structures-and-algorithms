...
const sortByChildren = (charArray) => {
  return charArray.sort((a, b) => {
    if (a.children.length === b.children.length) {
      return a.house.localeCompare(b.house); // Sort alphabetically by house name
    }
...
const containsWorld = (input) => {
  const pattern = /\bworld\b/; // Regular expression pattern to match the word 'world' with word boundaries
  return pattern.test(input);
};
...
const isCapitalized = (str) => {
  const pattern = /\b[A-Z][a-z]*\b/g; // Regular expression pattern to match words starting with a capital letter
  return str.match(pattern) || []; // Return an array containing all matches or an empty array if no matches found
};
...
const citiesAtoJ = (arr) => {
  const pattern = /^[A-J]/i; // Regular expression pattern matching letters A through J (case insensitive)
...
const matchMonth = (input) => {
  const pattern = /^(o|O)(c|C)(t|T)(ober)?$/; // Regular expression pattern to match October or Oct in different cases
  return pattern.test(input);
};
...
const noPunctuation = str => {
  const pattern = /\b\w+\s/g; // Regular expression pattern to find words with a space at the end
...
let hangman = (str) => {
  const pattern = /[aeiou]/gi; // Regular expression pattern to match vowels (case insensitive)
...
const findShells = (str) => {
  const pattern = /\b[sS]eashells?\b|\b[sS]ells\b|\b[Ss]hells\b/g; // Regular expression pattern to match the words
